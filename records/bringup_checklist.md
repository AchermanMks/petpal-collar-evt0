...
| F1 | 开机、日志、版本、开机原因 | ✅ | 2026-09-09 | raw/collar-evt-001/log_F1_win_20260909_062459.txt（2026-09-10 从 Luatools.zip 归档，已打码） | 日志：`petpal_evt0 001.000.001 collar-evt-001 evt0.0.1`、`lastReson 0 0 0 cold boot`、`hmeta Air780EGH A11`、features 全 false |
| F2 | SIM 注册、IP_READY、信号 | ⏳ | 2026-09-10 | raw/collar-evt-001/log_F2probe_*.txt；Windows 侧 log_F1_win_20260909_062459.txt（60 s，无卡，status 0） | Windows 烧录全程未插 SIM，F2 在 Windows 侧未执行（docs/09 §7.1）。Mac 直读日志：`+CPIN: NOT READY`，`rsrp 0 rssi 0 csq 0 status 0`，SIM 未被识别。需断电插好 SIM（拨动开关断 + 拔 USB）再开机。注册耗时 ___ s，RSRP ___ |
...
| 2026-09-09 | F2 | 烧录时未插 SIM，日志停在 waiting SIM/network | Luatools.zip 内日志核对：60 s 内 `status 0`、`+SOCCELL: 0,0,0,0,0`，全程无卡，无 IP_READY | 已确认（未执行） |
| 2026-09-10 | F2 | 出厂 V2044 固件 9/8 日志同样从未 IP_READY，并 3 次打印「拔卡」（raw/collar-evt-001/log_factory_V2044_win_20260908_013854.txt） | 当时可能也未插卡，不能单独证明卡座故障；说明读不到卡不是 petpal 脚本引入。已记入 fae_questions #9 | 记录 |
| 2026-09-10 | F0 | Windows 烧录时下载口是 COM15（下载模式重新枚举），与运行时 COM12/13 不同；9/8 03:54 电池 3014 mV 低于出厂关机阈值 3400 mV | 烧录/调试前先充电；docs/06 已补 | 记录 |
...
| 2026-09-10 | F0 | 每改一次 features 都要回 Windows 重烧 | 新增 console_app.lua + tools/usb_cmd.py：功能开关写 KV 覆盖，USB 命令切换后 reboot 生效；flash-kit v3 待烧，交接单 docs/10 | 待烧录验证 |
